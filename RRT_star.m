...
if strcmp('simpleMap',mapName)
    init_pos_rrt_star = round([init_pos(:,1),map.GridSize(1,2)-init_pos(:,2),0]);
    for i=1:size(goals,3)
        goals_poly = polyshape(goals(:,1,i),goals(:,2,i));
        [goal_centre_x,goal_centre_y] = centroid(goals_poly);
        goals_rrt_star(:,:,i) = round([goal_centre_x,map.GridSize(1,2)-goal_centre_y,0]);

        if i==1
            planner = plannerRRTStar(ss,sv, ContinueAfterGoalReached=true, MaxConnectionDistance=0.3);
